...
object LocationService {
  ...
  val employeeLocationParser = get[Int]("associate_id") ~ get[Int]("department_id") ~ get[String]("site") ~ get[Date]("event_time") ~ get[Int]("event_type") ~ AssociateService.associateParser map {
  ...
  def create(location: Location) = {
    val id: Option[Long] = DB.withConnection{ implicit c => 
      SQL("""INSERT INTO location_event(associate_id, department_id,site,event_time,event_type) values ({associate_id}, {department_id},{site},{event_time},{event_type})""")
  ...
  def getLastTwoAssociateLocationEvents(location: Location) = {
    val employeeLocations = DB.withConnection{ implicit c => 
      SQL("""SELECT loc.*, ass.first_name, ass.last_name 
             ...
             LIMIT 2""")
      .on("associate_id" -> location.associateId).as(employeeLocationParser *)
    }
  }
